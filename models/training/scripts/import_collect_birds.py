...
(default `data/server/collect/bird`), classifies each frame as `day` (color) or `ir`
(infrared/night) from image content, moves it into
`data/annotation/raw/tapo/{day,ir}/` with a clean name, and writes
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--source",
        type=Path,
        default=repo_root / "data" / "server" / "collect" / "bird",
