...
def dedupe_by_id(assets: Iterable[dict[str, Any]]) -> list[dict[str, Any]]:
    """Return ``assets`` with duplicate ids removed, keeping the first occurrence in order.

    Immich's ``/search/metadata`` pagination is page-numbered and does not dedupe, so a library
    mutation mid-scan can yield the same asset on two pages. Left in, the duplicate is enqueued as
    two independent work items that two consumers can process concurrently — and once one writes
    its record and ``delete_cached_thumbnail`` removes the cached file, the other can hit a vanished
    thumbnail and file a spurious error record that masks the real detection. Coalescing here (with
    the same ``str(asset["id"])`` coercion used for state keys) guarantees each id is scanned once.
    """
    seen: set[str] = set()
...
from aviary_immich.thumbnails import delete_cached_thumbnail
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--cache-dir", type=Path, default=Path("models/immich/data/cache"))
    parser.add_argument("--state-dir", type=Path, default=Path("models/immich/data/state"))
    ...
    parser.add_argument(
        "--keep-cache",
        action="store_true",
        help="Keep each downloaded thumbnail after it's processed instead of deleting it "
        "immediately (the cache then grows to the whole library).",
    )
...
def main() -> None:
    ...
    for account in config.accounts:
        ...
        with ExitStack() as stack:
            ...
            assets = dedupe_by_id(progress(client.iter_image_assets(page_size=args.page_size, limit=args.limit), desc=account.slug, unit="asset"))
