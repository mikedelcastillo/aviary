...
`models/immich/manifests/`. Each generated album's description is stamped with the date of the run
that last refreshed it (e.g. `Auto-generated by Aviary. Last updated 2026-06-09.`).

## Clear the thumbnail cache

`generate-albums` caches downloaded preview thumbnails under `models/immich/cache/thumbnails/`,
which can grow to many GB. It is a pure cache (re-fetched on demand), so it is safe to delete:

```bash
uv run --no-sync clear-cache            # delete the cache
uv run --no-sync clear-cache --dry-run  # just report how much would be freed
```

Scan state and manifests are left intact (deleting them would force a full re-scan). Pass
`--cache-dir` if you run with a non-default cache location.
