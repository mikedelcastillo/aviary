...
# in your shell profile and then `uv run generate-albums` works as-is.
#
...
echo
echo "Done. Run the app with:  uv run --no-sync generate-albums"
echo "(or 'export UV_NO_SYNC=1' once, then just 'uv run generate-albums')"
